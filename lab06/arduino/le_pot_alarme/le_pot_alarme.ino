...
int limiteAlarme = 100;              // limite do alarme
int valorPot = 0;
float voltsPot = 0;
int voltsPot_int = 0;
int voltsPot_int2 = 0;
// guarda o valor lido no potenciometro
...
void enviaDadosPelaSerial(){              // opcional, para debug
  ...
  Serial.print("; Volts lido: "); Serial.print(voltsPot);
...
void loop(){                       // Le o valor no potenciometro a cada 1 segundo
  ...
  voltsPot = valorPot*5.0/1024.0;
  voltsPot_int = static_cast<int>(voltsPot);
  voltsPot_int2 = static_cast<int>(voltsPot*100)-100*voltsPot_int;
  buffer[0] = valorPot & 0xFF;     // byte menos significativo
  buffer[1] = valorPot >> 8;       // byte mais significativo

  buffer[5] = voltsPot_int; 
  buffer[6] = voltsPot_int2;
